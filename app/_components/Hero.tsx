    ...
    //     <div className="relative z-10 container mx-auto flex flex-col-reverse lg:flex-row items-stretch justify-between px-4 py-5 sm:px-6 lg:px-12 xl:px-16 pt-12 sm:pt-16 lg:pt-20 xl:pt-24 pb-12 sm:pb-16 gap-6 sm:gap-8 lg:gap-12">
    ...
    //         >
    //         <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-5xl xl:text-6xl 2xl:text-7xl font-semibold text-[#2B2B2B] leading-tight">
    //             Where Tradition <br className="hidden sm:block" /> Meets Timeless Grace
    //         </h1>

    //         <p className="text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl text-[#5A5143] max-w-md sm:max-w-lg lg:max-w-xl leading-relaxed">
    //             Explore our exclusive oxidized jewelry designed to celebrate every shade of elegance.
    //         </p>

    //         <div className="flex justify-center lg:justify-start w-full pt-2">
    //             <Button className="bg-[#3E3124] text-[#F5E9D0] hover:bg-[#2E241B] transition-all duration-300 rounded-full px-6 sm:px-8 py-3 sm:py-4 text-sm sm:text-base md:text-lg">
    //             Explore More
    //             </Button>
    //         </div>
    //         </motion.div>

    //         {/* --- Right Image Section with geometric cutouts (reference style) --- */}
    //         <motion.div
    //         initial={{ opacity: 0, scale: 0.95 }}
    //         animate={{ opacity: 1, scale: 1 }}
    //         transition={{ duration: 1.2 }}
    //         className="relative w-full lg:w-1/2 flex justify-center items-center py-8 sm:py-12 lg:py-0"
    //         >
    //         {/* Large vertical pill */}
    //         <div className="absolute -z-20  w-52 sm:w-64 md:w-72 lg:w-96 h-80 sm:h-96 md:h-[420px] lg:h-[520px] bg-[#F9EAD9] rounded-[72px] shadow-md overflow-hidden flex items-start justify-center">
